...
export class News extends Component {
    ...
    async fetchArticles(page) {
        this.setState({loading: true});
        const url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=4fcb165c8f6848128486bdcc11dc5cc3&page=${page}&pageSize=16`;
        try {
    ...
    handleNextClick = () => {
        if (this.state.page < Math.ceil(this.state.totalResults / 16)) {
            this.fetchArticles(this.state.page + 1);
        }
    }
